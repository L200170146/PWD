...
<html>
	...
	<center>
		<?php
			...
			if ($Submit1) {
				if ($kodeb1 == "") {
					...
					$insert1 = "INSERT INTO jenis (kode_jenis, nama_jenis, keterangan_jenis) 
			...
			if ($_GET["hps1"]) {
				$kodeb1 = $_GET["hps1"];
				$hapus1 = "DELETE FROM jenis WHERE kode_jenis = '$kodeb1'";
				mysqli_query($conn, $hapus1);
				echo "<h3>Data berhasil di hapus</h3>";
			
			} elseif ($_GET["ubh1"]) {
				$kodeb1 = $_GET["ubh1"]; 
				$cari1 = "SELECT * FROM jenis WHERE kode_jenis='$kodeb1'";
				...
				while ($data1 = mysqli_fetch_row($hasil1)){
					echo "
						<tr>
							...
							<td>$data1[2]</td>
							<td>
								<a href='latihan.php?ubh1=$data1[0]'>Ubah</a>
								<a href='latihan.php?hps1=$data1[0]'>Hapus</a>
							</td>
						</tr>
					";
				}
		...
		<table border="1">
			...
			<?php
				$cari = "SELECT * FROM buku,jenis where buku.kode_jenis = jenis.kode_jenis";
				$hasil = mysqli_query($conn, $cari);
				while ($data = mysqli_fetch_row($hasil)){
					echo "
						<tr>
							...
							<td>$data[2]</td>
							<td>
								<a href='latihan.php?ubh=$data[0]'>Ubah</a>
								<a href='latihan.php?hps=$data[0]'>Hapus</a>
							</td>
						</tr>
					";
				}
